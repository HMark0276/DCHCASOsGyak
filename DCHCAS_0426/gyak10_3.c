...
int main() {
    ...
    if ((child = fork()) == 0) {
        ...
        {
            printf("szegmenst el kell kesziteni\n");
            shmflg = 00666 | IPC_CREAT;
            if ((shmid=shmget( key, size, shmflg)) < 0) 
            {
                perror("shmget rendszerhivas sikertelen\n");
                exit(-1);
            }
        } 
        else 
    ...
    else
    {
        ...
        do {
            printf("adja meg a parancs szamat\n");
            ...
            case 0: rtn = shmctl(shmid, IPC_STAT, buf);
                printf("segmens meret... %ld\n",buf->shm_segsz);
                printf("otolso shmop-os processz PID... %d\n ",buf->shm_lpid);
                break;
